...
def preproc_msp(msp_fp,keys,num_entries):
	...
	for raw_l in tqdm(raw_data_lines):
		...
		if raw_l == '':
			...
			# elif key == "Instrument" and raw_data_item["Instrument_type"] is None:
			# 	value = raw_l_split[1]
			# 	raw_data_item["Instrument_type"] = value
			elif key in keys:
				value = raw_l_split[1]
...
def preproc_nist_mol(mol_dp):
	...
	def proc_mol_file(mol_fp):
		...
		spectrum_id = mol_fn.lstrip("ID").rstrip(".MOL")
...
def merge_and_check(msp_df,mol_df,rename_dict):
	...
	if mol_df is None:
		...
		assert msp_df["spectrum_id"].isna().all()
		# check which ones can be parsed
		# mols = par_apply_series(msp_df["smiles"],data_utils.mol_from_smiles)
		# msp_df.loc[:,"smiles"] = par_apply_series(mols,lambda x: data_utils.mol_to_smiles(x,isomericSmiles=True))
		msp_df.loc[:,"spectrum_id"] = np.arange(msp_df.shape[0])
		spec_df = msp_df
	else:
		assert msp_df["smiles"].isna().all()
		assert not msp_df["spectrum_id"].isna().all()
		# merge with mol on spectrum_id
		msp_df = msp_df.drop(columns=["smiles"])
		spec_df = pd.merge(msp_df,mol_df,how="inner",on="spectrum_id")
...
if __name__ == "__main__":
	...
	parser.add_argument('--msp_file', type=str, required=True) #, default='nist_20/hr_nist_msms.MSP')
	parser.add_argument('--mol_dir', type=str, required=False) #, default='nist_20/hr_nist_msms.MOL')
	...
	parser.add_argument('--output_type', type=str, default="json", choices=["json","csv"])
	...
	# msp_df_fp = os.path.join(args.output_dp,"msp_df.json")
	# mol_df_fp = os.path.join(args.output_dp,"mol_df.json")
	...
	# save files
	spec_df_fp = os.path.join(args.output_dp,f"{args.output_name}.{args.output_type}")
